( function( global, $ ) {
    if( jQuery('#wbebaic_custom_css').length > 0 ){
        var custom_css_editor,
            syncCSS = function() {
                $( '#custom_css_textarea' ).val( custom_css_editor.getSession().getValue() );
            },
            loadAce = function() {
                custom_css_editor = ace.edit( 'wbebaic_custom_css' );
    ...
    if( jQuery('#wbebaic_custom_js').length > 0 ){
        var custom_js_editor,
            syncJS = function() {
                $( '#custom_js_textarea' ).val( custom_js_editor.getSession().getValue() );
            },
            loadAceJS = function() {
                custom_js_editor = ace.edit( 'wbebaic_custom_js' );
