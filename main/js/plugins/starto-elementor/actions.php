<?php
add_filter( 'posts_where', 'starto_title_like_posts_where', 10, 2 );
function starto_title_like_posts_where( $where, $wp_query ) {
    global $wpdb;
...
**/
add_action('wp_ajax_starto_ajax_search_result', 'starto_ajax_search_result');
add_action('wp_ajax_nopriv_starto_ajax_search_result', 'starto_ajax_search_result');

function starto_ajax_search_result() 
{
	global $wpdb;
	...
	if (strlen($_POST['keyword'])>0) {
	    ...
	    echo '<ul>';
	 	...
	 	if (have_posts()) : while (have_posts()) : the_post();
	 		...
	 		$course_title = starto_highlight_keyword($course_title, $_POST['keyword']);
